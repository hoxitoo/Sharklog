...
## UX/UI Аудит v2 (2026-06-06)

### Проблемы, выявленные при аудите

| # | Проблема | Статус |
|---|---|---|
| UX-1 | 6 вкладок таббара — перебор, мизинец не дотягивается | ✅ Исправлено |
| UX-2 | Emoji-иконки в навигации — не нативный вид на iOS/Android | ✅ Исправлено |
| UX-3 | Кнопка «+» скрыта в ScreenHeader, не всегда видна | ✅ Исправлено |
| UX-4 | Партнёры скрыты в Настройках — видный раздел закопан | ✅ Исправлено |
| UX-5 | AddBetScreen: 10+ полей всегда видны, 90% пользователей их не заполняют | ✅ Исправлено |
| UX-6 | Dashboard: 8 блоков без иерархии — информационный перегруз | ✅ Исправлено |
| UX-7 | Тилт-баннер нельзя закрыть — раздражает опытных пользователей | ✅ Исправлено |
| UX-8 | Нет ResponsibleGamblingBanner — требование App Store (Sports категория) | ✅ Исправлено |
| UX-9 | Нет haptic при тилте и сохранении ставки | ✅ Исправлено |

### Решения

**Drawer-навигация** — заменяет таббар. Преимущества: неограниченное число разделов без UX-деградации, hamburger — стандартный паттерн для мобильных приложений с боковым меню.

**FAB (Floating Action Button)** — всегда видна, не зависит от активного экрана. Это главное действие приложения — должно быть всегда доступно.

**Ionicons вместо emoji** — `@expo/vector-icons` входит в Expo SDK. Векторные иконки: масштабируются без артефактов, поддерживают активное/неактивное состояние через filled/outline варианты, выглядят нативно на обеих платформах.

**Collapsible extra fields в AddBetScreen** — принцип progressive disclosure: показывать только то, что нужно большинству. Редкие поля (стратегия, букмекер, турнир, заметки) раскрываются по запросу. При редактировании ставки с уже заполненными доп. полями — раскрываются автоматически.

**Dashboard hierarchy** — информационная архитектура по убыванию важности: текущий период → главный KPI (P&L) → детали (W/L, винрейт) → динамика (график) → контекст (серия, банк) → история (последние ставки). Тепловая карта перенесена в collapsed-секцию как «детальный анализ».

**Tilt dismiss** — UX-паттерн «показать предупреждение один раз в день». Опытный пользователь знает о тилте — баннер каждый сеанс раздражает. Dismiss сохраняется в AsyncStorage с датой; новая серия проигрышей сбрасывает dismiss.
