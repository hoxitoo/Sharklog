...
| Сверка банка с балансом букмекера | — | ✅ |
| Калькулятор Келли | — | ✅ |
| Дневной лимит ставок | — | ✅ |
| Тилт-алерт | ✅ (фикс. 3) | ✅ (настраиваемый) |
| Дневник + трекер настроения | ✅ | ✅ |
| Инсайты по турнирам | ✅ | ✅ |
| Тап по турниру/команде → все её ставки | ✅ | ✅ |
| Лучший / худший турнир и команда | ✅ | ✅ |
| Разбивка по годам (турниры + «без турнира») | ✅ | ✅ |
| Дисциплина киберспорта в статистике (CS2, Dota 2…) | ✅ | ✅ |
| Колесо действий на ставке (тап по карточке) | ✅ | ✅ |
| Везение или решения (σ-отклонение результата) | — | ✅ |
| Соблюдение плана по банкроллу | — | ✅ |
| Дублирование ставки в один тап | ✅ | ✅ |
| Билдер стратегий | — | ✅ |
| Импорт CSV / Excel | ✅ | ✅ |
| Экспорт CSV / JSON | ✅ | ✅ |
| 5 статусов ставки | ✅ | ✅ |
...
**3 типа движения по банку:** Пополнение / Вывод / Сверка — последняя выравнивает банк
под реальный баланс у букмекера без фейкового депозита. Учти, что букмекер списывает
ставку сразу при постановке, а приложение — только при расчёте, поэтому пока есть
незакрытые ставки, баланс у бука меньше банка в приложении ровно на их сумму.
...
```bash
cd packages/core && npx vitest run       # 98 unit-тестов
...
| Тесты | Jest (25 smoke) | Vitest (98 core + 40 smoke) + Playwright (22 E2E) |
