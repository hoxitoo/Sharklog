...
describe('BalanceChart.pickIndex', () => {
  // plotW 204 − 2×4 inset = 196 usable, 5 points ⇒ 49px apart.
  const W = 204;
  const N = 5;

  it('lands on the first point at the left inset', () => {
    expect(pickIndex(4, W, N)).toBe(0);
  });

  it('lands on the last point at the right inset', () => {
    expect(pickIndex(200, W, N)).toBe(N - 1);
  });

  it('steps one point per spacing', () => {
    expect(pickIndex(4 + 49, W, N)).toBe(1);
    expect(pickIndex(4 + 98, W, N)).toBe(2);
    expect(pickIndex(4 + 147, W, N)).toBe(3);
  });

  it('snaps to the nearer point between two', () => {
    expect(pickIndex(4 + 24, W, N)).toBe(0);  // just under half a step
    expect(pickIndex(4 + 25, W, N)).toBe(1);  // just over
  });
