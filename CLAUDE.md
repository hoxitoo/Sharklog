...
- **Подписка на стор — ТОЛЬКО с селектором**: `useBetsStore((s) => s.bets)`, никогда `const { bets } = useBetsStore()`. Без селектора это подписка на весь объект состояния, который zustand заменяет при каждой записи, — любое изменение перерисовывает подписчика. Так карточка ставки (по одной на видимую строку) и навигатор (рендерит активный экран) перерисовывались на каждую запись, а экран заодно пересчитывал статистику. Действия стора стабильны по ссылке, их тоже берут селектором.
- **У бара и заголовка строки — ОДНА величина.** Общий `SliceRow` рисует бар от |P&L|, а крупным числом справа пишет ROI, и на реальных данных они спорят вслух: хоккей (+71.5%, 25 108 ₽) и киберспорт (−3.6%, −25 019 ₽) дают бары одной длины — глаз читает «эти двое равны», пока число говорит о разнице в двадцать раз. В `SportBreakdown` (Аналитика) бар и заголовок — это деньги, ROI ушёл в подпись, а бар расходится от НУЛЕВОЙ ОСИ, поэтому знак несёт форма, а не только цвет. Деньги наверху не случайно: ROI на 12 ставках — это дисперсия, а рубли — то, что произошло; доля оборота стоит рядом, потому что 90% WR на 12 ставках и на 206 — разные утверждения.
- **Тяжёлые `calc*` — в `useMemo`**, и проверять по пути рендера, а не грепом: полный `calcDashboard` может стоять строкой ниже уже обёрнутого вызова, а `isInTilt` — пересчитываться на каждое нажатие в поиске.
- **Значение контекста мемоизируй** (`DrawerContext`): контекст обходит `React.memo`, и свежий объект в `value` перерисовывает всех потребителей на каждый рендер провайдера.
- **`AppText` кэширует разрешение начертания** (`fontCache` в `theme/typography`): каждый `Text` спрашивает его на каждом рендере, и это единственная стоимость, растущая ровно с размером дерева.
- **У длинных списков ограничивай `windowSize`** (9): дефолт RN — 21 экран контента, при 229 ставках это ~110 живых карточек, у каждой свой `PanResponder`. Остальные пропсы батчинга не трогай: `maxToRenderPerBatch` ниже дефолтных 10 только добавляет пустоты на броске, а 50 мс — и так дефолт.
...
navigation/
  ...
  DrawerNavigator.tsx      — кастомный анимированный drawer (заменяет таббар с v2)
                             Экраны НЕ размонтируются при переходе: раз посещённый остаётся
                             смонтированным (`visited`), скрытый — `display:'none'` (вне
                             раскладки) и обёрнут в <Freeze> из react-freeze. Заморозка
                             обязательна: React.memo не спасает от ХУКОВОГО обновления, и без
                             неё одно закрытие ставки перерисовывало бы все шесть деревьев с
                             графиками. Монтируем лениво, а не все шесть сразу — иначе вся
                             стоимость переезжает в запуск.
                             Все переходы идут через showScreen(), не через setScreen.
...
screens/
  BetsScreen/              — SectionList + quick-result W/L/R/C (cashout)
                             ...
                             Свёртка ОДНОСТОРОННЯЯ: листание вверх панель НЕ возвращает. Назад
                             только двумя намеренными действиями — доскроллить до верха или
                             тапнуть по трею. Иначе мелкий рывок вверх накрывал полноразмерной
                             панелью строки, которые читаешь.
  ...
  AnalyticsScreen/         — РЕДИЗАЙН: hero-состояние сверху + collapsible «Расширенная статистика».
                             Hero: P&L + столбцы за период (PnlBars, тап по столбцу; выбранный
                             столбец подсвечивается ПОЛОСОЙ-КОЛОНКОЙ, а не только притушенными
                             соседями — у месяца с маленьким столбиком тушить нечего),
                             серии, рекорды, «По виду спорта» (SportBreakdown), время ставок
                             (донат 6 промежутков + топ-4 часа с P&L, 12h/24h через
                             uses12HourClock()), CLV-карточка. Всё PRO via ProGate.
                             Карточки «Прошлый месяц» НЕТ: hero уже показывает и итог месяца,
                             и те же помесячные столбцы — это был дубль. Вместе с ней ушли
                             WR месяца и дельта к предыдущему месяцу.
                             Расширенная (collapsible): betType/bookmaker/strategy/odds/day срезы —
                             sport оттуда УБРАН, иначе карточка дублируется.
                             Карточки «Соблюдение плана» на мобилке НЕТ (снята по просьбе: при
                             банке меньше ставки доля уходила в >999% и блок не читался).
                             `calcPlanCompliance` в core остаётся — её использует десктоп
  BankrollScreen/          — история транзакций показывает первые 5, остальные под
                             «Ещё N транзакций»: она растёт без предела и уносила калькулятор
                             Келли на экран вверх. Разворот односторонний — экран стековый и
                             состояние сбрасывается при возврате.
                             «Пополнить / Вывести / Сверить»; сверка = ввод реального баланса бука,
                             ...
                             СКРАББЕР: тап или протяжка по графику → вертикальная линия, точка на
                             кривой и пилюля «день · баланс», плюс суммы депозита/вывода этого дня
                             (раньше было видно, что событие было, но не какое). PanResponder
                             забирает жест на touch-down (чтобы работал одиночный тап), но
                             `onShouldBlockNativeResponder: () => false` отдаёт его ScrollView —
                             иначе график становится мёртвой зоной 150 pt, через которую не
                             пролистать. Именно этот проп, а НЕ
                             `onPanResponderTerminationRequest`: на Android захват JS-респондера
                             дёргает `requestDisallowInterceptTouchEvent(true)` у всех предков, и
                             ScrollView жест вообще не видит; запрос на завершение там не
                             спрашивают — жест отменяют. В координатах во время протяжки нужен
                             `pageX` минус origin, снятый на touch-down: Android пересчитывает
                             `locationX` относительно того View, что под пальцем СЕЙЧАС, поэтому
                             уход за левый край в колонку подписей сдвигал показания вперёд на
                             несколько дней. На iOS обоих эффектов нет — в симуляторе не
                             воспроизведётся. Арифметика подбора точки — `pickIndex` в
                             `utils/chartScale` (там нет рантайма RN, поэтому она под тестом):
                             отступ `PLOT_INSET` надо разворачивать точно, иначе все показания
                             сползают к середине — правдоподобно и неверно;
...
components/
  ...
  FilterPicker.tsx         — фильтр как кнопка + нижний шит со списком опций.
                             ...
                             ВЕСЬ шит обёрнут в Pressable со stopPropagation — иначе тап по
                             его собственным полям (40 pt под последней опцией) уходит на
                             подложку и закрывает вместо выбора. dragY сбрасывается при
                             ОТКРЫТИИ, а не при закрытии: сброс во время затухания
                             телепортирует шит обратно в кадр
