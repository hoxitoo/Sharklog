...
- **Фильтр списка ставок** — `BetsFilter { date?, tournament?, team?, from?, year?, noTournament? }`. Команду матчь через `betBacksTeam()` (то же правило, что у `calcByTeam`), иначе количество в списке не сойдётся с числом на плитке инсайтов.
...
```bash
# Тесты
cd packages/core && npx vitest run        # 205 unit-тестов core
