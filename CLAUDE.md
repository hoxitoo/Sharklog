...
navigation/
  RootNavigator.tsx        — Stack: Drawer + AddBet (modal) + Bankroll + StrategyBuilder + Partners
                             + BetsFilter. Весь стек обёрнут в <BetsQueryProvider>: экран
                             фильтров и список ставок здесь СОСЕДИ, а не родитель и потомок,
                             поэтому ни один не может держать это состояние за другого, а
                             вернуть его через route-параметры значило бы несериализуемый
                             колбэк. В betsStore ему не место — это то, на что смотрят сейчас,
                             а не то, что надо пережить перезапуск
...
screens/
  BetsScreen/              — SectionList + quick-result W/L/R/C (cashout)
                             ...
                             Справа от них — квадратная кнопка расширенных фильтров с бейджем
                             числа критериев; открывает экран BetsFilter.
                             ПОИСК ИЩЕТ И ПО ТУРНИРУ, не только по событию и исходу: имя на
                             диске несёт сезон («StarLadder StarSeries Fall 26»), и набранное
                             по памяти «StarLadder» отдавало «ничего не найдено» по турниру,
                             у которого ставки есть.
                             Под полем поиска — подсказки из реальных данных (турниры и
                             команды, до 4). Показываются по СОДЕРЖИМОМУ поля, а не по фокусу:
                             blur приходит раньше, чем долетает вызвавший его тап, и скрытие по
                             blur размонтировало строку из-под пальца. Выбор подсказки делает
                             запрос точным совпадением, а `suggest` точные совпадения
                             выбрасывает — список закрывается сам
  BetsFilterScreen/        — расширенный фильтр списка: турнир, команда/спортсмен, коэф (от-до),
                             дата (от-до). Правится ЧЕРНОВИК, применяется по «ОК» — выход
                             назад оставляет список каким был. Складывается со статусом и
                             сортировкой, не заменяет их. Чипы подсказок — из реальных данных.
                             Логика — `utils/betsQuery.ts` (без рантайма RN, поэтому под
                             тестом): половинчатая дата (`2026-0`) НЕ фильтрует, иначе список
                             пустеет на середине набора; перепутанные границы меняются местами,
                             а не дают пустоту; запятая читается как десятичная точка — это то,
                             что предлагает клавиатура; пустой запрос возвращает ТОТ ЖЕ массив,
                             чтобы не ронять мемоизацию группировки.
                             Активный фильтр виден чипом над списком (`describeBetsQuery`) —
                             забытый фильтр с другого экрана это то, из-за чего короткий список
                             читается как потерянные данные. Чип на символах (`× ≥1.5`,
                             `01.09 →`), без слов: переводы ему не нужны, и он короче
  ...
  AnalyticsScreen/         — РЕДИЗАЙН: hero-состояние сверху + collapsible «Расширенная статистика».
                             ...
                             Под шапкой графика — строка с WR периода и дельтой к предыдущему
                             (▲/▼). Строка рисуется ВСЕГДА с фиксированной высотой: появляясь
                             только по выбору, она сдвигала график вниз ровно под пальцем,
                             который только что тапнул по столбцу. У текущего периода в
                             заголовке стоит «· идёт» — стрелка, сравнивающая половину месяца
                             с целым, читается как обвал.
                             ...
                             Карточки «Прошлый месяц» НЕТ: hero показывает и итог месяца,
                             и те же помесячные столбцы, и WR с дельтой — это был дубль.
  ...
  BankrollScreen/          — история транзакций показывает первые 5, остальные под
                             ...
                             ПЕРЕКЛЮЧАТЕЛЬ СЕРИИ «P&L / Банк» вместо заголовка карточки,
                             по умолчанию P&L. На счёте, который часто пополняют и выводят,
                             линия банка — это пила из переводов: она проходит всю высоту
                             графика в день, когда ставок не было. P&L отвечает «выигрываю ли
                             я», банк — «сколько там лежит»; это разные вопросы, поэтому
                             переключатель, а не переделка. В режиме P&L с линии снимаются
                             метки депозитов/выводов и строки потоков в пилюле: депозит эту
                             линию не двигал, и точка на ней утверждала бы обратное.
...
utils/
  stats.ts            — calcDashboard, calcByField, calcByOddsRange, calcByDayOfWeek,
                        ...
                        betBacksTeam(bet, team, { partial }) — тот же предикат наружу, для
                        фильтра списка ставок. partial ослабляет сравнение до подстроки — для
                        поля, в которое ПЕЧАТАЮТ, а не плитки, по которой тапнули («Ак» должно
                        находить «Ак Барс»). Точная форма остаётся дефолтом: именно она держит
                        длину отфильтрованного списка равной числу на плитке инсайтов
